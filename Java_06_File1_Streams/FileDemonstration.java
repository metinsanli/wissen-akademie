	...
	public static void analyzePath(String path) {
		...
		if (name.exists()) {
			System.out.printf("%s%s\n%s\n%s\n%s\n%s%s\n%s%s\n%s%s\n%s%s\n%s%s", name.getName(), " exists", (name.isFile() ? "is a file"
					: "is not a file"), (name.isDirectory() ? "is a directory" : "is not a directory"), (name.isAbsolute() ? "is absolute path"
					: "is nor absolute path"), "Last modified: ", name.lastModified(), "Length: ", name.length(), "Path\t\t: ", name.getPath(),
